...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
/* The variable internal is declared before the nested function so its scope is the entire parent function */
...
function summation(summationParam) {
  let sum = 0;
  for(let i =1; i <= summationParam; i++){sum = sum + i;}
  return sum
...
const zooAnimals = [
  ...
  */
  function animalNames(nameParam){
    let newArray = [];
  ...
  */
  function lowerCaseNames(lowerParam){
    let newArray = []
    newArray = lowerParam.map(function(x){
  ...
  */
  ...
  function lowPopulationAnimals(popParam){
    let newArray =[];
    newArray = popParam.filter(function(x){
  ...
  */
  ...
  function USApop(usaParam){
    let newArray = []
    newArray = usaParam.map(function(x){ return x.population})
    let sum = newArray.reduce(function (previousValue, currentValue) {
...
function greeting(greet1,greet2){
   return `Hello ${greet1} ${greet2}, nice to meet you!`
  }
...
let step4Obj={length:4,width:5,height:5};
let cuboid = new CuboidMaker(step4Obj)
...
class CuboidMakerTwo{
  ...
  surfaceArea(){return 2*((this.length*this.width)+(this.length*this.height)+(this.height*this.width))}
  }
  let cuboidTwo = new CuboidMakerTwo(step4Obj)
